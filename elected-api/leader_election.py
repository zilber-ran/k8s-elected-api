...
class LeaderElection:
    ...
    def try_acquire_or_renew(self):
        logging.info('[STARTED] try_acquire_or_renew')
        ...
        logging.info('lock_status:\n\t{}\nold_record:\n\t{}'.format(lock_status, old_election_record.__dict__.items()))
        ...
        logging.info('leader_election_record:\n\t{}'.format(leader_election_record))
        # A lock is not created with that name, try to create one
        if not lock_status:
            if old_election_record.body:
                logging.info('[OLD-RECORED-BODY] {}'.format(old_election_record.body))
        ...
        logging.info('[-DONE-] try_acquire_or_renew')
        return self.update_lock(leader_election_record)
